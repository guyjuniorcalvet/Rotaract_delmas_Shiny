...
server <- function(input, output, session) {
  ...
  db_pool <- tryCatch({ config <- get_db_config(); pool <- dbPool(drv = RMariaDB::MariaDB(), dbname = config$dbname, host = config$host, user = config$user, password = config$password, port = config$port); message("✅ Pool de connexions à la base de données créé avec succès."); pool }, error = function(e) { message("❌ ÉCHEC CRITIQUE DE LA CRÉATION DU POOL: ", e$message); showNotification("Erreur irrécupérable de connexion à la base de données.", type="error", duration=NULL); NULL })
  onSessionEnded(function() { if (!is.null(db_pool)) { message("🚪 Session terminée. Fermeture du pool de connexions."); poolClose(db_pool) } })
  ...
  observeEvent(res_auth$user, {
    ...
    roles_config <- list("Développeur" = c("vues", "saisie", "requetes_simples", "requetes_sql"), "Président(e)" = c("vues", "saisie", "requetes_simples", "requetes_sql"), "PastPrésident(e)" = c("vues", "saisie", "requetes_simples"), "Vice-président(e)" = c("vues", "saisie"), "Secretaire" = c("saisie"), "Secrétaire" = c("saisie"))
    ...
    simple_query_choices <- c(
      ...
      "Membres par Poste" = "membres_poste",
      "Membres par Profession" = "membres_profession",
      "Membres par Pays" = "membres_pays",
      "Anniversaires du mois" = "membres_anniversaire",
      "Activités dans une période" = "activites_periode", 
      "Cotisations d'un membre" = "cotisations_membre"
    )
  ...
  get_pays_choices <- get_distinct_choices("Pays")
  ...
  observeEvent(input$submit_ajout, { req(db_pool, input$table_select_saisie); table_name <- input$table_select_saisie; query <- NULL; is_valid <- TRUE; if (table_name == "Membres") { if (any(sapply(c(input$ajout_nom, input$ajout_prenom, input$ajout_genre, input$ajout_poste, input$ajout_statut, input$ajout_profession, input$ajout_ville, input$ajout_pays, input$ajout_email, input$ajout_telephone), function(x) x == ""))) { is_valid <- FALSE } else { query <- sqlInterpolate(db_pool, "INSERT INTO Membres (Nom, Prenom, Genre, Date_naissance, Poste, Statut, Date_debut, Profession, Ville, Etat, Pays, Zip_code, Email, Telephone, Groupe_sanguin, Facebook, Instagram, Twitter, Tiktok, Linkedln) VALUES (?nom, ?prenom, ?genre, ?date_naissance, ?poste, ?statut, ?date_debut, ?profession, ?ville, ?etat, ?pays, ?zip_code, ?email, ?telephone, ?groupe_sanguin, ?facebook, ?instagram, ?twitter, ?tiktok, ?linkedin)", nom = input$ajout_nom, prenom = input$ajout_prenom, genre = input$ajout_genre, date_naissance = input$ajout_date_naissance, poste = input$ajout_poste, statut = input$ajout_statut, date_debut = input$ajout_date_debut, profession = input$ajout_profession, ville = input$ajout_ville, etat = input$ajout_etat, pays = input$ajout_pays, zip_code = input$ajout_zip_code, email = input$ajout_email, telephone = input$ajout_telephone, groupe_sanguin = input$ajout_groupe_sanguin, facebook = input$ajout_facebook, instagram = input$ajout_instagram, twitter = input$ajout_twitter, tiktok = input$ajout_tiktok, linkedin = input$ajout_linkedln) } } else if (table_name == "Activites") { if (any(sapply(c(input$ajout_theme, input$ajout_heure_debut, input$ajout_heure_fin, input$ajout_lieu, input$ajout_type_activite), function(x) x == ""))) { is_valid <- FALSE } else { query <- sqlInterpolate(db_pool, "INSERT INTO Activites (Theme, Date_activite, Heure_debut, Heure_fin, Présentiel, Lieu, Ville_activite, Pays_activite, Type_activite) VALUES (?theme, ?date_activite, ?heure_debut, ?heure_fin, ?presentiel, ?lieu, ?ville_activite, ?pays_activite, ?type_activite)", theme = input$ajout_theme, date_activite = input$ajout_date_activite, heure_debut = input$ajout_heure_debut, heure_fin = input$ajout_heure_fin, presentiel = as.integer(input$ajout_presentiel), lieu = input$ajout_lieu, ville_activite = input$ajout_ville_activite, pays_activite = input$ajout_pays_activite, type_activite = input$ajout_type_activite) } } else if (table_name == "Cotisations") { if (is.null(input$ajout_id_membre_cot) || input$ajout_id_membre_cot == "" || is.null(input$ajout_montant) || input$ajout_type_cotisation == "") { is_valid <- FALSE } else { query <- sqlInterpolate(db_pool, "INSERT INTO Cotisations (ID_membre, Date_cotisation, Montant, Type_cotisation, Mode_paiement) VALUES (?id_membre, ?date_cotisation, ?montant, ?type_cotisation, ?mode_paiement)", id_membre = input$ajout_id_membre_cot, date_cotisation = input$ajout_date_cotisation, montant = input$ajout_montant, type_cotisation = input$ajout_type_cotisation, mode_paiement = input$ajout_mode_paiement) } } else if (table_name == "Presence") { if (is.null(input$ajout_id_membre_pres) || input$ajout_id_membre_pres == "" || is.null(input$ajout_id_activite_pres) || input$ajout_id_activite_pres == "" || input$ajout_statut_pres == "") { is_valid <- FALSE } else { query <- sqlInterpolate(db_pool, "INSERT INTO Presence (ID_membre, ID_activite, Statut) VALUES (?id_membre, ?id_activite, ?statut)", id_membre = input$ajout_id_membre_pres, id_activite = input$ajout_id_activite_pres, statut = input$ajout_statut_pres) } }; if (!is_valid) { showModal(modalDialog(title = "Champs Invalides", p("Les champs avec * sont requis."), easyClose = TRUE)); return() }; tryCatch({ dbExecute(db_pool, query); showNotification(paste("Ajout réussi à la table", table_name), type = "message"); data_trigger(Sys.time()) }, error = function(e) { showModal(modalDialog(title = "Erreur SQL", p("L'ajout a échoué. Détails de l'erreur :"), tags$pre(e$message), easyClose = TRUE)) }) })
  ...
  # --- 8. PAGE : RECHERCHES COMPLEXES (SQL) ---
