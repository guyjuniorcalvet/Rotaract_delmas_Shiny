...
server <- function(input, output, session) {
  ...
  db_pool <- tryCatch({
    ...
    showNotification("Erreur irrécupérable de connexion à la base de données. L'application ne peut pas fonctionner.", type="error", duration=NULL)
    NULL
  })
  ...
  output$footer_last_update_text <- renderText({ 
    paste("Version", APP_VERSION, "| Dernière mise à jour :", format(Sys.time(), "%d %B %Y à %H:%M")) 
  })
}
